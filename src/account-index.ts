...
import type { CopilotAccountIndex, CopilotAccountRecord, CopilotUsageSnapshot, UsageWindow } from "./types.js";
...
export async function readAccountIndex(indexPath = resolveAccountIndexPath()): Promise<CopilotAccountIndex> {
  try {
    const raw = await fs.readFile(indexPath, "utf8");
    const parsed = JSON.parse(raw) as CopilotAccountIndex;
...
export async function writeAccountIndex(index: CopilotAccountIndex, indexPath = resolveAccountIndexPath()): Promise<void> {
  await atomicWriteJson(indexPath, index);
}

function mergeAccount(
  existing: CopilotAccountRecord | undefined,
  currentAuth: CopilotAccountRecord,
): CopilotAccountRecord {
  return {
    ...existing,
    ...currentAuth,
...
function sortAccounts(accounts: CopilotAccountRecord[]): CopilotAccountRecord[] {
...
export async function syncAccountIndex(params?: {
  ...
  if (currentAuth) {
    ...
    const currentRecord: CopilotAccountRecord = {
      id,
  ...
  const refreshed: CopilotAccountRecord[] = [];
  ...
  const nextIndex: CopilotAccountIndex = {
    version: 1,
    updatedAt: now,
...
export function renderDashboard(index: CopilotAccountIndex): string {
